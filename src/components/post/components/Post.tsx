...
import MarkdownRenderer from '@/common/UIElements/markdownRenderer/MarkdownRenderer';
import SocialShareLinks from '@/common/UIElements/SocialShareLinks';
import buildImage from '@/common/utils/cloudinary';
...
const Post: React.FC<PostProps> = ({ post }) => {
  ...
  const [sideContentVisibility, setSideContentVisibility] = useState<string>('none');
  const [isLessThan1180px] = useMediaQuery('(max-width: 1180px)');
  const textColor = useColorModeValue('text', '#fff');
  const { hasCopied, onCopy } = useClipboard(`${process.env.NEXT_PUBLIC_SITE_URL}/${post.slug}`);
  ...
  useEffect(() => {
    let handleProgress = () => {
      const totalScroll =
        ...
        (document.getElementById('head-content')?.clientHeight || 0);
      const windowHeight =
        document.getElementById('blog-content')?.clientHeight || 0;
      const scroll = `${totalScroll / windowHeight}`;
      setHeight(parseFloat(scroll));
    };
    ...
    <>
      ...
      <Box my={10}>
        <AdSense
          ...
          adSlot={siteConfig.adsense.slots.pageTop} adFormat={''}        />
      </Box>
